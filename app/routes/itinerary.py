...
def list_itineraries():
    try:
        if current_user.is_authenticated:
            # Show public itineraries and itineraries from friends
            public_users = User.query.filter_by(profile_visibility=True).all()
            friend_ids = [friend.id for friend in current_user.friends]
            itineraries = Itinerary.query.filter(
                or_(
                    User.profile_visibility == True,  # Public users' itineraries
                    Itinerary.user_id.in_(friend_ids),  # Friends' itineraries
...
def view_itinerary(id):
    try:
        ...
        all_activities = Activity.query.all()  # You might want to filter this based on visibility too
